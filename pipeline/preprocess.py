...
def saveOneHotEncoder(one_hot_encoder: OneHotEncoder, file_name: str, out_path: str) -> None:
    """
...
def _chi2_anova(df, cat, num):
    ...
    return chi2(pd.get_dummies(df[cat]), y_disc)[1].min(), \
...
def feature_engineering(df:pd.DataFrame, nombre:str, target_col: str, low_var: float, high_corr: float, one_hot_encoder: OneHotEncoder = None) -> Tuple[pd.DataFrame, OneHotEncoder]:
    logging.info(f"··· FE: {nombre.upper()} ···")
    df["Fecha"]=pd.to_datetime(df["Year"].astype(str)+"-"+df["Mes"].astype(str).str.zfill(2))
    ...
    df=df.join(pd.DataFrame(ohe.fit_transform(df[categorical_cols]),
    ...
    df.drop(columns=_remove_high_corr(df.select_dtypes("number"), target_col, high_corr), inplace=True)
...
def main():
    ...
    for file in file_params:
        ...
        for teq_type in ["teq100", "teq"]:
            ...
            train_df, one_hot_encoder = feature_engineering(train_df,
                                file_name_and_type, 
                                target_col,
                                low_var,
                                high_corr)
            ...
            test_df, _ = feature_engineering(test_df,
                                ...
                                target_col,
                                low_var,
                                high_corr, one_hot_encoder)
            
            # ═════════════════════════════════ Save transformed dataframes ═════════════════════
            saveTransformedDataFrame(train_df, out_path, f"{file_name_and_type}_train_features.csv")
            saveTransformedDataFrame(test_df, out_path, f"{file_name_and_type}_test_features.csv")
